...
- (void)commonInit {
    ...
    self.topView.backgroundColor= [[UIColor blackColor] colorWithAlphaComponent:.0f];
    
    self.handleImageView = [[UIImageView alloc] initWithFrame:CGRectZero];
    [self.topView addSubview:self.handleImageView];
    self.handleImageView.backgroundColor = [UIColor colorWithRed:0.0 green:0.478 blue:1.0 alpha:1.0];
    self.handleImageView.layer.cornerRadius = 3.0;
    self.handleImageView.layer.masksToBounds = YES;
...
- (void)layoutSubviews {
    ...
    self.topView.frame  = CGRectMake(0, 0, self.frame.size.width, 20);
    self.handleImageView.frame = CGRectMake((self.topView.frame.size.width - 48) / 2.0, (self.topView.frame.size.height - 6) / 2.0, 48, 6);
...
- (void)showInView:(UIView *)view offset:(CGFloat)offset {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.frame = CGRectMake(0, view.frame.size.height - self.minHeight - offset, self.frame.size.width, self.minHeight);
        self.contentsView.frame = CGRectMake(0, 20, self.frame.size.width, self.frame.size.height - 20);
    }];
}

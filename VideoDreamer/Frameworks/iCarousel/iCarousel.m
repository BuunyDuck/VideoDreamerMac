...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeCoverFlow2:
        {
            ...
            CGFloat z = (CGFloat)fabs(clampedOffset) * -_itemWidth * 0.5f;
...
- (NSInteger)circularCarouselItemCount
{
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeInvertedWheel:
        {
            ...
            CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:1.0f];
            CGFloat width = _vertical ? self.bounds.size.height: self.bounds.size.width;    
//            count = MIN(MAX_VISIBLE_ITEMS, MAX(/*12*/16, ceilf(width / (spacing * _itemWidth)) * M_PI));
            count = MIN(MAX_VISIBLE_ITEMS, MAX(_numberOfItems, ceilf(width / (spacing * _itemWidth)) * M_PI));
...
{    
    ...
    if (_numberOfItems > 0)
    {
        [self scrollToItemAtIndex:_numberOfItems-1 animated:YES];
    }
}
...
- (void)startDecelerating
{
    ...
    _startTime = CACurrentMediaTime();
    _scrollDuration = (NSTimeInterval)(fabs(distance) / fabs(0.5f * _startVelocity));
    ...
    else if (_decelerating)
    {
        ...
        if (time == (CGFloat)_scrollDuration)
        {
            ...
            else
            {
                ...
                _toggleTime = (NSTimeInterval)(currentTime - MAX_TOGGLE_DURATION * fabs(difference));
    ...
    if (_scrollEnabled)
    {
        switch (panGesture.state)
        {
            ...
            default:
            {
                ...
                if (!_wrapEnabled && _bounces)
                {
                    factor = (CGFloat)(1.0f - fminf(fabs(_scrollOffset - [self clampedOffset:_scrollOffset]), _bounceDistance) / _bounceDistance);
                }
...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    if (_scrollEnabled)
    {
        ...
        if (!_wrapEnabled && _bounces)
        {
            factor = (CGFloat)(1.0f - fminf(fabsf(_scrollOffset - [self clampedOffset:_scrollOffset]), _bounceDistance) / _bounceDistance);
        }
